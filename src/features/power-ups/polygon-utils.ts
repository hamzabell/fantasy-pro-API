 ...
 */
export function maticToWei(maticAmount: string): string {
	// 1 MATIC = 10^18 wei
	const weiPerMatic = BigInt(10 ** 18);
	const maticBigInt = BigInt(Math.floor(parseFloat(maticAmount) * 10 ** 18));
 ...
 */
export function weiToMatic(weiAmount: string): string {
	// 1 MATIC = 10^18 wei
	const weiPerMatic = BigInt(10 ** 18);
 ...
 */
export function getPolygonScanUrl(txHash: string, network: string = 'mainnet'): string {
 ...
 * @returns The scanner URL
 */
export function getPolygonAddressUrl(address: string, network: string = 'mainnet'): string {
	const baseUrl = network === 'mainnet' ? 'https://polygonscan.com' : `https://${network}.polygonscan.com`;
 ...
 * @returns The scanner URL
 */
export function getPolygonTokenUrl(tokenAddress: string, tokenId?: string, network: string = 'mainnet'): string {
	const baseUrl = network === 'mainnet' ? 'https://polygonscan.com' : `https://${network}.polygonscan.com`;
	if (tokenId) {
 ...
 */
export function formatTransactionDate(timestamp: Date): string {
