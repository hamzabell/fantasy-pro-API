...
leaguePowerUpsApp.openapi(getLeaguePowerUpsRoute, async (c) => {
	...
	const leagueMembershipPowerUps = await prisma.fantasyLeagueMembershipPowerUp.findMany({ 
		where: { 
	...
	return c.json({
		...
		powerUps: leagueMembershipPowerUps.map(membershipPowerUp => ({
			...membershipPowerUp.powerUpUsage.powerUp,
			transactionId: membershipPowerUp.powerUpUsage.transactionId,
			usedBy: {
				email: membershipPowerUp.powerUpUsage.user.email,
				teamName: membershipPowerUp.fantasyLeagueMembership.teamName || 'Unknown'
			},
			createdAt: membershipPowerUp.powerUpUsage.createdAt.toISOString(),
			updatedAt: membershipPowerUp.powerUpUsage.updatedAt.toISOString(),
		})),
	}, 200);
});
