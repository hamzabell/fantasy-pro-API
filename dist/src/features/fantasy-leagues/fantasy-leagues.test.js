...
describe("Fantasy Leagues", () => {
    describe("Create Fantasy League", () => {
        ...
        test("given an authenticated user tries to create a fantasy league with invalid data types: it should throw a 400 error", () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            vi.spyOn(supabase.auth, 'getUser').mockImplementation(() => mockSupabase.auth.getUser());
            // Invalid data types
            const response = yield app.request('/api/fantasy-leagues', Object.assign(Object.assign({ method: 'POST' }, createAuthHeaders()), createBody({
        ...
        test("given an aunthenticated user tries to create a fantasy league with a draft date in the past: it should throw a 400 error", () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    describe('GET leagues', () => {
        test('given an authenticated user tries to get all leagues: it should return all public leagues with owner details and calculated fields', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            vi.spyOn(supabase.auth, 'getUser').mockImplementation(() => mockSupabase.auth.getUser());
            const response = yield app.request('/api/fantasy-leagues', Object.assign({ method: 'GET' }, createAuthHeaders()));
        ...
        test('given an authenticated user tries to filter leagues by stake value: it should return only leagues with that stake value', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues?stake=100', Object.assign({ method: 'GET' }, createAuthHeaders()));
            expect(response.status).toBe(200);
            const actual = yield response.json();
            // Filter to only the leagues we created in this test
        ...
        test('given an authenticated user tries to sort leagues by creation date: it should return leagues in the correct order', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const responseAsc = yield app.request('/api/fantasy-leagues?sortBy=createdAt&sortOrder=asc', Object.assign({ method: 'GET' }, createAuthHeaders()));
            ...
            const responseDesc = yield app.request('/api/fantasy-leagues?sortBy=createdAt&sortOrder=desc', Object.assign({ method: 'GET' }, createAuthHeaders()));
        ...
        test('given an authenticated user tries to filter leagues by isMember=true: it should return only leagues the user is a member of', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues?isMember=true', Object.assign({ method: 'GET' }, createAuthHeaders()));
            expect(response.status).toBe(200);
            const actual = yield response.json();
            // Filter to only the leagues we created in this test
            const testLeagues = actual.leagues.filter((league) => [savedLeague1.id, savedLeague2.id].includes(league.id));
        ...
        test('given an authenticated user tries to filter leagues by isMember=false: it should return only leagues the user is not a member of', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues?isMember=false', Object.assign({ method: 'GET' }, createAuthHeaders()));
            expect(response.status).toBe(200);
            const actual = yield response.json();
            // Filter to only the leagues we created in this test
            const testLeagues = actual.leagues.filter((league) => [savedLeague1.id, savedLeague2.id].includes(league.id));
        ...
        test('given an authenticated user tries to search leagues by name: it should return only leagues matching the search term', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues?search=Premier', Object.assign({ method: 'GET' }, createAuthHeaders()));
            expect(response.status).toBe(200);
            const actual = yield response.json();
            // Filter to only the leagues we created in this test
            const testLeagues = actual.leagues.filter((league) => [savedLeague1.id, savedLeague2.id].includes(league.id));
        ...
        describe('GET leagues/:id', () => {
            test('given an authenticated user tries to get a leagues details via its id: it should return the details of that league', () => __awaiter(void 0, void 0, void 0, function* () {
                ...
                const mockSupabase = mockSupabaseAuthSuccess(savedUser);
                vi.spyOn(supabase.auth, 'getUser').mockImplementation(() => mockSupabase.auth.getUser());
                const response = yield app.request(`/api/fantasy-leagues/${savedLeague.id}`, Object.assign({ method: 'GET' }, createAuthHeaders()));
                expect(response.status).toBe(200);
                const actual = yield response.json();
                const expected = {
                    message: "League Retrieved Successfully",
                    league: Object.assign(Object.assign({}, league), { id: savedLeague.id, createdAt: savedLeague.createdAt.toISOString(), updatedAt: savedLeague.updatedAt.toISOString() })
                };
                expect(actual).toEqual(expected);
                // clean up
            ...
            test('given an authenticated user tries to get a leagues details via its id that does not exist: it should return a 404 error', () => __awaiter(void 0, void 0, void 0, function* () {
                ...
                const mockSupabase = mockSupabaseAuthSuccess(savedUser);
                vi.spyOn(supabase.auth, 'getUser').mockImplementation(() => mockSupabase.auth.getUser());
                const response = yield app.request(`/api/fantasy-leagues/${faker.string.uuid()}`, Object.assign({ method: 'GET' }, createAuthHeaders()));
    ...
    describe('Join Fantasy League', () => {
        ...
        test('given an authenticated user tries to join a league that is ongoing or closed: it should return a 400 error', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
            ...
            const savedClosedLeague = yield saveFantasyLeagueToDatabase(closedLeague);
            // Mock supabase Auth
            const mockSupabase = mockSupabaseAuthSuccess(user);
            vi.spyOn(supabase.auth, 'getUser').mockImplementation(() => mockSupabase.auth.getUser());
            ...
            // Clean up
            yield deleteFantasyLeagueFromDatabaseById(savedClosedLeague.id);
            yield deleteUserFromDatabaseById(user.id);
            yield deleteUserFromDatabaseById(owner.id);
        }));
        test('given an authenticated user tries to join a league with an ongoing gameweek: it should return a 400 error', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
            ...
            const savedOngoingLeague = yield saveFantasyLeagueToDatabase(ongoingLeague);
            // Mock supabase Auth
            const mockSupabase = mockSupabaseAuthSuccess(user);
            vi.spyOn(supabase.auth, 'getUser').mockImplementation(() => mockSupabase.auth.getUser());
            ...
            // Clean up
            yield deleteFantasyLeagueFromDatabaseById(savedOngoingLeague.id);
            yield deleteUserFromDatabaseById(user.id);
            yield deleteUserFromDatabaseById(owner.id);
        }));
    ...
    describe('League Gameweek Lifecycle', () => {
        test('given a league is created when the current gameweek is ongoing: it should be created for the current gameweek', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
        ...
        test('given an authenticated user tries to create a fantasy league when the current gameweek is ongoing: it should automatically assign the current gameweek', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
        ...
        test('given an authenticated user tries to create a fantasy league when the current gameweek is a past gameweek: it should automatically assign the current gameweek', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
        ...
        test('given an authenticated user tries to create a fantasy league without specifying a gameweek: it should automatically assign the current gameweek', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
        ...
        test('given an authenticated user tries to create a fantasy league: it should correctly calculate prize distribution for different numbers of winners', () => __awaiter(void 0, void 0, void 0, function* () {
            // Setup
            const user = yield saveUserToDatabase({
                id: faker.string.uuid(),
                email: faker.internet.email(),
    ...
    describe('League Table', () => {
        test('given a league id: it should return the league table sorted by points in descending order', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request(`/api/fantasy-leagues/${savedLeague.id}/table`, Object.assign({ method: 'GET' }, createAuthHeaders()));
        ...
        test('given a non-existent league id: it should return a 404 error', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request(`/api/fantasy-leagues/${faker.string.uuid()}/table`, Object.assign({ method: 'GET' }, createAuthHeaders()));
            expect(response.status).toBe(404);
            // Clean up
            yield deleteUserFromDatabaseById(user.id);
        }));
    ...
    describe('League History', () => {
        test('given an authenticated user: it should return the user\'s league history', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues/history', Object.assign({ method: 'GET' }, createAuthHeaders()));
        ...
        test('given an authenticated user: it should allow filtering league history by league id', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request(`/api/fantasy-leagues/history?leagueId=${savedLeague1.id}`, Object.assign({ method: 'GET' }, createAuthHeaders()));
            // For now, we expect a 404 since the endpoint doesn't exist yet
            expect(response.status).toBe(404);
            // Clean up
            yield deleteFantasyLeagueFromDatabaseById(savedLeague1.id);
        ...
        test('given an authenticated user: it should allow filtering league history by status', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues/history?status=ongoing', Object.assign({ method: 'GET' }, createAuthHeaders()));
            // For now, we expect a 404 since the endpoint doesn't exist yet
            expect(response.status).toBe(404);
            // Clean up
            yield deleteFantasyLeagueFromDatabaseById(savedLeague1.id);
        ...
        test('given an authenticated user: it should allow sorting league history by creation date', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues/history?sortBy=createdAt&sortOrder=asc', Object.assign({ method: 'GET' }, createAuthHeaders()));
            // For now, we expect a 404 since the endpoint doesn't exist yet
            expect(response.status).toBe(404);
            // Clean up
            yield deleteFantasyLeagueFromDatabaseById(savedLeague1.id);
        ...
        test('given an authenticated user: it should allow searching league history by name', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request('/api/fantasy-leagues/history?search=Premier', Object.assign({ method: 'GET' }, createAuthHeaders()));
            // For now, we expect a 404 since the endpoint doesn't exist yet
            expect(response.status).toBe(404);
            // Clean up
            yield deleteFantasyLeagueFromDatabaseById(savedLeague1.id);
    ...
    describe('League Position', () => {
        test('given a league id for a league the user is part of: it should return the user\'s position in the league', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request(`/api/fantasy-leagues/${savedLeague.id}/position`, Object.assign({ method: 'GET' }, createAuthHeaders()));
            // Expect a successful response since the endpoint now exists
            expect(response.status).toBe(200);
            const actual = yield response.json();
        ...
        test('given a non-existent league id: it should return a 404 error', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const response = yield app.request(`/api/fantasy-leagues/${faker.string.uuid()}/position`, Object.assign({ method: 'GET' }, createAuthHeaders()));
            // For now, we expect a 404 since the endpoint doesn't exist yet
            expect(response.status).toBe(404);
            // Clean up
            yield deleteUserFromDatabaseById(user.id);
